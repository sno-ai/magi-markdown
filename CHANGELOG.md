...
- Conformance suite: 35 fixtures (valid + invalid) bound to spec rule IDs; runner at `scripts/validate-conformance.mjs` enforces schema validity plus the cross-field signature/integrity equality rule.
- `REGISTRY.md`: vendor namespaces, standard `requires` keys, reserved Sigstore OIDC issuers, reserved Rekor instances.
