...
1. A text editor.
2. The `@mda/cli` reference implementation, or — if you'd rather not install anything — the [manual workflow](https://github.com/sno-ai/mda/blob/main/docs/manual-workflow.md), which uses only `sha256sum` and `cosign`.
...
```bash
npx @mda/cli compile pdf-tools.mda --target SKILL.md
```
...
```bash
npx @mda/cli compile pdf-tools.mda --target SKILL.md --sign
```

The output `SKILL.md` gets `metadata.mda.integrity.digest` (a JCS-canonicalized hash of the canonical bytes) and `metadata.mda.signatures[]` (DSSE-enveloped signature with Rekor inclusion proof and Fulcio certificate metadata). A verifier rederives the digest, looks up Rekor, verifies the cert chain, and applies an OIDC identity allow-list against operator policy.
...
<CardGroup cols="2">
  ...
  <Card title="Manual workflow" icon="terminal" href="https://github.com/sno-ai/mda/blob/main/docs/manual-workflow.md">
    Hand-author and sign without the reference CLI. Just a text editor, `sha256sum`, and `cosign`.
  </Card>
