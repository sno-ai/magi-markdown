# MDA (.mda) to MDX (.mdx) Conversion

<Note>
**Scope.** This page covers the broader MDA superset including `ai-script` JSON blocks. **`ai-script` is out of v1.0 spec scope** (see [CHANGELOG](/changelog/log)). The v1.0-conformant feature set is rich frontmatter + typed footnote relationships + cryptographic identity. Conversion guidance below is still useful for legacy `.mda` content; for the v1.0-aligned path, start at the [Specification](/mdx/specification).
</Note>

This document outlines the technical approach for converting MDA (`.mda`) files into MDX (`.mdx`) format. The goal is to leverage the strengths of both formats: MDA for structured AI guidance and MDX for rich, interactive web content.

The conversion process focuses on mapping the three core MDA components (Front Matter, `ai-script` blocks, Footnote Relationships) into functional MDX equivalents.

## 1. Front Matter Conversion

MDA's YAML Front Matter can be directly mapped to MDX's front matter.
...
title: "Introduction to MDA"
description: "Explains the MDA format."
...
*   A script or pre-processor during the build step should parse the MDA YAML front matter and inject it into the MDX file's front matter section.
*   **Case Convention:** Decide on a consistent case convention for front matter keys within the MDX context. While MDA uses kebab-case (e.g., `doc-id`), JavaScript/TypeScript typically uses camelCase (e.g., `docId`). Converting to camelCase (`frontmatter.docId`) is often more idiomatic for component logic. However, keeping kebab-case and accessing via bracket notation (`frontmatter['doc-id']`) is also valid. This conversion, if done, should happen during the pre-processing step. Ensure consistency throughout your MDX components.
*   Field names might need case conversion (e.g., kebab-case in MDA to camelCase in MDX JavaScript/TypeScript environments) for easier consumption.
...
MDA uses JSON within Markdown footnotes to define typed relationships. In MDX, these can be converted into enhanced links or custom React components.
...
[^spec1]: `{"rel-type": "citation", "doc-id": "SPEC-UUID-123", "rel-desc": "Cites the core specification"}`
...
This document references specifications<Tooltip content='{"rel-type": "citation", "doc-id": "SPEC-UUID-123", "rel-desc": "Cites the core specification"}'><sup>[1]</sup></Tooltip> and related guides<Tooltip content='{"rel-type": "related", "source-url": "https://example.com/guide", "rel-desc": "Related implementation guide"}'><sup>[2]</sup></Tooltip>.
...
This document references specifications <DocLink docId="SPEC-UUID-123" relType="citation" relDesc="Cites the core specification" /> and related guides <DocLink sourceUrl="https://example.com/guide" relType="related" relDesc="Related implementation guide" />.
...
This technical document provides a blueprint for the MDA-to-MDX conversion process, highlighting the key considerations for each component type. The actual implementation will depend heavily on the specific MDX framework and tooling used.
