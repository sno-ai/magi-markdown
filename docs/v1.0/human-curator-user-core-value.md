...
v1.0 specifies the contract. It does not ship a turnkey verifier. Operators currently glue `cosign` and a JCS library themselves. Sigstore-path verification depends on Fulcio and Rekor reachability. Reserved Sigstore OIDC issuers in `REGISTRY.md` are recognition, not blanket trust. Unsigned authoring still works; signing is opt-in per artifact.
