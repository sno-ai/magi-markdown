...
v1.0 freezes the contract. The spec, the target schemas, the integrity rules, the signature envelope, the capability declarations, the 35-fixture conformance suite. All of that is locked at `v1.0.0-rc.1`.

The consumer side — the verifiers, resolvers, indexers, harnesses that actually enforce or route through that contract — is mostly nascent. This page is the gap, written down honestly.

## The nine gaps

1. **No central artifact registry** (§0.2). MDA does not host or mandate a registry of `.mda` artifacts; resolution remains operator-defined.
...
3. **No working v1.0 dependency resolver implementation.** Resolver behavior is normative (§03-3.3) — refuse load on digest mismatch, prefer highest satisfying SemVer — but the reference implementation matures across `v1.0.0-rc.N` tags. The final `1.0.0` releases when the conformance suite passes 100%.
...
6. **`CLAUDE.md` remains a Tier 2 stub target in v1.0** (§06-targets/claude-md §06-1). It is recognized as a target but its schema is intentionally minimal pending observed cross-runtime adoption.
7. **`MCP-SERVER.md` is Tier 2 in v1.0** (§06-targets/mcp-server-md). Graduation to Tier 1 requires observable demand and ≥2 independent implementations (§06-9).
...
9. **No coverage of Cursor MDC, Windsurf rules, Continue, Aider, or `*.instructions.md`** as compile targets in v1.0. MDA targets the agentskills.io v1 ecosystem (`SKILL.md`) and the AAIF-aligned ecosystem (`AGENTS.md`, `MCP-SERVER.md`); other 2026 skill-format families still require parallel maintenance.
...
These exist as spec contracts but have no shipped implementation in v1.0. The reference TypeScript CLI at `packages/mda/` (npm `@mda/cli`) ships incrementally across `v1.0.0-rc.N` tags. The final `1.0.0` releases when `@mda/cli` passes 100% of the conformance suite. Third-party implementations are equally welcome — the contract does not privilege the reference.
...
These are design choices, not gaps to fill. v1.0 doesn't host a registry — resolution stays operator-controlled. v1.0 doesn't take responsibility for Sigstore reachability — the contract specifies what to do when Sigstore is reachable, operators decide whether and how to wire it. v1.0 targets agentskills.io v1 plus AAIF, not every skill-format family in the 2026 ecosystem. Items 1 and 8 will not "graduate"; they describe the architectural boundary of what MDA controls. Item 9 may expand in a future minor release if observed adoption justifies new compile targets, per the §0.9 versioning policy. v1.0 makes no commitment.
