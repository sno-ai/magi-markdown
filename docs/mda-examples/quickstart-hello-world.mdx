...
<Note>
**About these examples.** The examples in this section illustrate the broader MDA superset — including `ai-script` JSON blocks and pre-v1.0 frontmatter shapes. **`ai-script` is out of v1.0 scope** and the v1.0 frontmatter places MDA-extended fields under `metadata.mda.*`, not at the top level. For v1.0-conformant authoring, start at [Quickstart](/quickstart) and the [Specification](/mdx/specification). The examples here remain useful for understanding patterns in legacy `.mda` content.
</Note>
