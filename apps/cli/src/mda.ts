...
export function resolveTarget(file: string, target: MaybeTarget): { ok: true; target: Target } | { ok: false; result: (command: string, file: string) => CommandResult } {
  ...
  if (extname(base) === ".md") {
    return {
      ok: false,
      result: (command, f) => usage(command, `Ambiguous Markdown target for ${f}; pass --target source|SKILL.md|AGENTS.md|MCP-SERVER.md`, { file: f })
    };
  }
  return {
    ok: false,
    result: (command, f) => usage(command, `Could not auto-detect target for ${f}; pass --target source|SKILL.md|AGENTS.md|MCP-SERVER.md`, { file: f })
  };
}
...
export function canonicalizeFromFile(file: string, target: Target, sidecar: string | null): { ok: true; bytes: Buffer; files: string[] } | { ok: false; exitCode: ExitCode; diagnostics: Diagnostic[]; files: string[] } {
...
export function compileTargets(frontmatter: Record<string, unknown>, body: string, targets: Target[], outDir: string, includeIntegrity: boolean) {
  ...
  return diagnostics.length ? { ok: false as const, diagnostics, planned: outputs.map((o) => o.path) } : { ok: true as const, outputs, diagnostics };
}
...
function jcs(value: unknown): string {
  ...
  if (isRecord(value)) {
    return `{${Object.keys(value).sort().filter((key) => value[key] !== undefined).map((key) => `${JSON.stringify(key)}:${jcs(value[key])}`).join(",")}}`;
  }
  return "null";
}
